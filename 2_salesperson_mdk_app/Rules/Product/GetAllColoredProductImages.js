...
function ExecuteColoredDownloadImage(prodBinding, fileName){
  console.log(`Preparing to downloading Product: (${prodBinding.ProductIndex}) - ${prodBinding.MachineColorsID}`);
  ...
  return this.executeAction('/Mahlwerk_Sales/Actions/Products/DownloadColoredProductImage.action').then(result => {
...
export default function GetAllColoredProductImages(context) {
  ...
  // which mean each Product entity is also a media
	return context.read('/Mahlwerk_Sales/Services/Mahlwerk_Sales_MDK.service', 'MachineColorsSet', []).then(result => {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (!fileExists(fileName)) {
          let prodBinding = {
            ...
            'ProductIndex': i,
            'MachineColorsID': prod.MachineColorsID,
          ...
          latestPromise = latestPromise.then(ExecuteColoredDownloadImage.bind(pageProxy, prodBinding, fileName));
        }
      }
...
return latestPromise.then(function(){
  return this.executeAction("/Mahlwerk_Sales/Actions/Products/AllProductImagesDownloaded.action").then(function(){
    this.evaluateTargetPathForAPI('#Page:CoffeeMachine/#Control:SectionedTable0').redraw();
