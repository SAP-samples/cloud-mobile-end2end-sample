...
public class OfflineODataController {
    ...
    public func configureOData(sapURLSession: SAPURLSession, serviceRoot: URL, onboardingID: UUID) throws {
        ...
        self.odataService = OdataService(provider: offlineODataProvider)
    }
    ...
    public func openOfflineStore(synchronize: Bool, completionHandler: @escaping (Error?) -> Void) {
        if !self.isOfflineStoreOpened {
            ...
            self.odataService.open { error in
                if let error = error {
    ...
    private func configureDefiningQueries(on offlineODataProvider: OfflineODataProvider) throws {
        ...
        do {
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.materialPositionSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.materialPositionSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.jobSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.jobSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.orderEventsSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.orderEventsSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.userSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.userSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.machineSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.machineSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.orderSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.orderSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.materialSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.materialSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.taskSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.taskSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.customerSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.customerSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.toolPositionSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.toolPositionSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.stepSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.stepSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.addressSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.addressSet).selectAll(), automaticallyRetrievesStreams: false))
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: OdataServiceMetadata.EntitySets.toolSet.localName, query: DataQuery().from(OdataServiceMetadata.EntitySets.toolSet).selectAll(), automaticallyRetrievesStreams: false))
        } catch {
    ...
    private func downloadOfflineStore(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.odataService.download { error in
            if let error = error {
    ...
    private func uploadOfflineStore(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.odataService.upload { error in
            if let error = error {
